...
bool Client::StartNetwork()
{
	...
	while (true)
	{
		Text::ChooseDesc<Text::RefVector> text_info(LanguagePack::text[LanguagePack::multiplayer_client_lobby]);
		...
		if (option == Multiplayer::active_games)
		{
			...
			Text::ChooseDesc<Text::RefVector> text_info(active_games);
			text_info.text_align = TextAlign::left;
			text_info.starting_index = 0;
			text_info.spacing = 0;
			text_info.point_of_reference = submenu_position;
