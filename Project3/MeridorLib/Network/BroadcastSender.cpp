...
{
	void MeridorNetwork::BroadcastSender::Broadcasting()
	{
		...
		while (this->thread_active)
		{
			...
			for (int i = 0; i < 255; ++i)
			{
				InetPtonA(AF_INET, ("192.168." + std::to_string(i) + ".255").c_str(), &sock_addr.sin_addr.s_addr);
			...
			if (hamachi)
			{
				InetPtonA(AF_INET, "25.255.255.255", &sock_addr.sin_addr.s_addr);
				if (!(sendto(broadcast_socket, host_name, sizeof(host_name), 0, reinterpret_cast<sockaddr*>(&sock_addr), sizeof(sock_addr)) < 0))
				{
					result = true;
				}
	...
	void MeridorNetwork::BroadcastSender::Stop()
	{
		thread_active = false;
	...
	void MeridorNetwork::BroadcastSender::Start(bool hamachi)
	{
		this->hamachi = hamachi;
		thread_active = true;
		main_thread = std::thread(&BroadcastSender::Broadcasting, this);
	}
	MeridorNetwork::BroadcastSender::~BroadcastSender()
	{
		Stop();
	}
}