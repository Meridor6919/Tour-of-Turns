...
{
	void MeridorNetwork::BroadcastReceiver::Receiving()
	{
		...
		addr.sin_port = htons(MeridorNetwork::Constants::port_number);
		...
		while (thread_active)
		{
			...
			if (hostent != nullptr && recv_result != -1)
			{
				...
				if (!finded)
				{
					hosts_broadcasting.push_back(intercepted_host);
				}
	...
	void MeridorNetwork::BroadcastReceiver::Stop()
	{
		thread_active = false;
	...
	void MeridorNetwork::BroadcastReceiver::Start(bool hamachi)
	{
		this->hamachi = hamachi;
		thread_active = true;
		main_thread = std::thread(&BroadcastReceiver::Receiving, this);
	}
	std::vector<std::string> MeridorNetwork::BroadcastReceiver::GetHostVector()
	{
		return hosts_broadcasting;
	}
	void MeridorNetwork::BroadcastReceiver::ResetHostVector()
	{
		broadcast_mutex.lock();
	...
	MeridorNetwork::BroadcastReceiver::~BroadcastReceiver()
	{
		Stop();
	}
}